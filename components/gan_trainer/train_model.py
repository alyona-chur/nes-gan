...
def main(configuration: Dict[Any, Any]):
    ...
    name = configuration.gan_training.experiment_name.lower().replace(' ', '_')
    logger = LoggerManager(log_file_path=get_absolute_path(
        DEFAULT_LOGGING_DIR, ROOT_DIR) / Path(f'{name}.log'))
    ...
    train_data_dir = data_dir / Path('train')
    valid_data_dir = data_dir / Path('valid')
    tmp_dir = get_absolute_path(TMP_DATA_DIR, ROOT_DIR)
    if tmp_dir.is_dir():
    ...
    trainer = GanTrainer(generator, configuration.gan_training.noise_vector_len,
                         ...
                         DataProcessor(data_processor_config, logger), tmp_dir, logger)
